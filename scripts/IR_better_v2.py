...
def best_cosine(w1) :
	cosine_sim = 0
	best_similarity =''
	vocs = list(set(myvocs.keys()).intersection(vocab))
	for w in vocs:
		cs = cosine_similarity(myvocs[w1], myvocs[w])
		if cs > cosine_sim:
			cosine_sim= cs
			best_similarity = w
...
def search(words, nRank = 7):
	...
	rank = 1;
	for pId, n in nlargest(50, score.iteritems(), key=itemgetter(1)):
		print str (pId) + '|' + str (n)
		rank += 1
		if rank > nRank:
			print ('\n')
...
with open(dir_path + '/w2v_myvoc.pickle', 'rb') as handle:
    myvocs = pickle.load(handle)
